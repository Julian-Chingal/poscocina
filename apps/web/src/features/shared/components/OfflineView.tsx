...
export const OfflineView: React.FC<OfflineViewProps> = ({ onRetry }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6 text-center select-none">
      <Card className="max-w-md w-full bg-slate-900 border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <Card className="bg-slate-950/60 rounded-2xl border-slate-800/80 p-3.5 mb-6 text-left text-xs space-y-1.5 font-mono text-slate-400">
        ...
        <Button
          onClick={handleRetry}
          disabled={retrying}
          className="w-full py-6 rounded-2xl bg-orange-600 hover:bg-orange-500 active:scale-98 text-white font-bold text-sm shadow-lg shadow-orange-600/20 transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
        >
