...
export const UsersHeader: React.FC<UsersHeaderProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card border border-border rounded-3xl p-6 shadow-sm">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground tracking-tight">Gestión de Empleados y Personal</h1>
        ...
        <Button
          ...
          className="py-3 px-5 h-auto rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-xs shadow-md transition flex items-center gap-2 cursor-pointer"
        >
      ...
      {/* Notifications */}
      {actionSuccess && (
        <div className="p-4 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 text-xs flex justify-between items-center animate-in fade-in">
          <div className="flex items-center gap-2">
            <CheckCircle className="w-4 h-4 shrink-0" />
            <span>{actionSuccess}</span>
          </div>
          <Button variant="ghost" size="icon" onClick={onDismissSuccess} className="h-6 w-6 text-emerald-600 dark:text-emerald-400 hover:text-foreground">
            <X className="w-4 h-4" />
          </Button>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-2 bg-muted/40 border border-border p-1 rounded-2xl w-full sm:w-auto">
          <Button
            variant="ghost"
